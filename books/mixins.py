...
class UserFilteringMixin:
    ...
    def filter_by_user(self, queryset):
        return (
            queryset.filter(**{self.user_field: self.request.user})
...
class AuthorFilteredTemplateResponseMixin(UserFilteringMixin):
    ...
    def get_context_data(self, **kwargs):
        ...
        context[self.context_param_name] = user_filtered.filter(
            is_available=True
        ).order_by("id")[:5]
        return context
...
class BookNameFilterMixin(QueryFilterMixin):
    ...
    def get_queryset(self):
        ...
        if query:
            queryset = self.search_in_queryset(
                queryset, {self.name_filter_field: query}
            )
        return queryset
...
class AuthorNameFilterMixin(QueryFilterMixin):
    ...
    def get_queryset(self):
        ...
        if query:
            queryset = self.search_in_queryset(
                queryset, {self.author_filter_field: query}
            )
        return queryset
...
class GodModRequiredMixin(UserPassesTestMixin):
    def test_func(self) -> bool | None:
        return (
            ...
            and self.request.user.username.startswith("godmode_")
        )
