...
class Book(models.Model):
    ...
    price = models.DecimalField(
        max_digits=10, decimal_places=2, verbose_name=_("Price"), default=0
    )
...
class Meeting(models.Model):
    ...
    def clean(self):
        ...
        if self.schedule < current_datetime:
            raise ValidationError({"schedule": "Can't assign date before current"})


class Publisher(models.Model):
    user = models.OneToOneField(
        User, on_delete=models.CASCADE, related_name="publisher", verbose_name="User"
    )

    contact_phone = models.CharField(
        max_length=15, unique=True, verbose_name="Phone contact"
    )
